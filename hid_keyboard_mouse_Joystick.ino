...
void loop() 
{
  ...
  if (Serial.available())
  {
    ...
    //First successful syntax, doing it the hard way
    //keycode[0] = (uint8_t)0x4F; //Right Arrow;
    //blehid.keyboardReport((uint8_t)0x0, keycode);

    keycode[0] = KEY_ARROW_RIGHT;
    blehid.keyboardReport(KEY_NONE, keycode); 
    //blehid.keyboardReport(KEY_NONE, KEY_ARROW_RIGHT); // This function call REALLY wants an array as the second param.  Can't figure out how to just pass the key code . . .

    keycode[0] = KEY_ARROW_LEFT;
    //blehid.keyboardReport(KEY_NONE, keycode);

    keycode[0] = KEY_ARROW_DOWN;
    //blehid.keyboardReport(KEY_NONE, keycode);

    keycode[0] = KEY_ARROW_UP;
    //blehid.keyboardReport(KEY_NONE, keycode);

    keycode[0] = KEY_PAGE_UP;
    //blehid.keyboardReport(KEY_NONE, keycode);

    keycode[0] = KEY_PAGE_DOWN;
    //blehid.keyboardReport(KEY_NONE, keycode);

    Serial.println("Buttons");
  
    blehid.mouseButtonPress(MOUSE_BUTTON_LEFT);
    //Serial.println(blehid.mouseReport(BLE_CONN_HANDLE_INVALID, MOUSE_BUTTON_LEFT, 0, 0, 0, 0) );

    blehid.mouseButtonPress(MOUSE_BUTTON_RIGHT);
    //Serial.println(blehid.mouseReport(BLE_CONN_HANDLE_INVALID, MOUSE_BUTTON_RIGHT, 0, 0, 0, 0) );
